...
/*
 * FIXME, xhci_ring_to_sgtable() code by Wesley Cheng,
 * slighty modified by Mathias, Not reviewed in detail
 */
			      ...
			      struct usb_host_endpoint *host_ep)
{
	...
	/*
	 * FIXME, ring deq, enq and other info should be reset, flushed, zeroed,
	 * or synced
	 */
...
/**
 * xhci_sideband_endpoint_buffer - gets the endpoint transfer buffer address
 * @sb: sideband instance for this usb device
 * @host_ep: usb host endpoint
 *
 ...
 */
struct sg_table *
xhci_sideband_endpoint_buffer(struct xhci_sideband *sb,
			      struct usb_host_endpoint *host_ep)
{
	struct xhci_virt_ep *ep;
	unsigned int ep_index;
...
/**
 * xhci_sideband_event_buffer - return the event buffer for this device
 * @sb: sideband instance for this usb device
 *
 ...
 */
struct sg_table *
xhci_sideband_event_buffer(struct xhci_sideband *sb)
{
	if (!sb->ir)
		return NULL;
...
/**
 * xhci_sideband_register - register a
